...
public class BoardController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "regist":
			...
			BoardRepository.getInstance().regist(vo); //객체 받아서 메서드 불러옴
			//글 등록완료.
		...
		case "list": 
			 ...
			 * 여기서 sendRedirect를 하면 안되는 이유.
			   ...
			   응답이 바로 나가면서 기존의 request 객체가 소멸해버린다. 
			   여기서는 forward방식을 사용해서 request를 원하는 jsp파일로 전달해서
			   그쪽으로 응답이 나갈 수 있도록 처리해야 한다. 
			...
			//request는 requestDispatcher객체타입이기에
			dp.forward(request, response);
			//dp는 request와 response를 가지고 경로로 나간다. 
			break;
		}
